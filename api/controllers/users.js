...
exports.get_user_by_username = (req, res, next) =>
{
    ...
    User.findOne({username: {$regex: new RegExp("^" + sanitize(req.params.username).toLowerCase(), "i")}})
...
{
    User.find({username: {$regex: new RegExp("^" + sanitize(req.params.username).toLowerCase(), "i")}})
        .then(result =>
        {
            ...
            DummyUser.find({username: {$regex: new RegExp("^" + sanitize(req.params.username).toLowerCase(), "i")}})
                .then(result =>
                {
                    if(result !== null)
                    {
...
exports.forgot_password_email = (req, res, next) =>
{
    ...
    User.findOne({$or : [{email: sanitize(req.body.user)}, {username: {$regex: new RegExp("^" + sanitize(req.body.user).toLowerCase(), "i")}}]})
        ...
        .then(result =>
        {
            ...
            const url = `https://mywatchlists.netlify.app/forgotPassword/${verificationToken}`;
                    ...
                    bcrypt.hash(sanitize(req.body.password), 10, (err, hash) =>
                    {
                        ...
                        } else
                        {
                            const dummyUser = new DummyUser({
                                ...
                                password: hash,
                                userProfilePic: req.file !== undefined ? cloudinary.url(sanitize(req.body.username) + "-profilePic") : "https://mywatchlist-apiv2.herokuapp.com/usersProfilePics/default.jpg",
                            });
                            ...
                            const url = `https://mywatchlists.netlify.app/verification/${verificationToken}`;
...
{
    DummyUser.findOne({username: {$regex: new RegExp("^" + sanitize(req.params.username).toLowerCase(), "i")}})
        .select('email')
        .then(result => {
            res.status(200).json(
                {
